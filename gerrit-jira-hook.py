...
log_path = os.path.join(os.environ.get("GERRIT_SITE", "/var/gerrit"), "hooks/hooks.log")
FORMAT = "%(asctime)-12s %(levelname)-8s %(message)s"
log.basicConfig(format=FORMAT, filename=log_path, level=log.DEBUG)

config_path = os.path.join(
    os.environ.get("GERRIT_SITE", "/var/gerrit"), "hooks/gerrit-jira-hook.config"
)
...
jira_log_all = config.get("gerrit-jira-hook", "jira_log_all")
if jira_log_all.lower() == "true":
    JIRA_LOG_ALL = True
else:
    JIRA_LOG_ALL = False

GERRIT_URL = config.get("gerrit-jira-hook", "gerrit_url")
...
def init():
    try:
        ...
        if PROJECTS[0] == "All-Projects" or (
            values["project"] != None and values["project"] in PROJECTS
        ):
            jira_hook(values)
        else:
...
def find_issue_identifiers(change, jira_instance):
    ...
    proc = subprocess.Popen(
        ...
        + "@"
        + GERRIT_HOST
        + " gerrit query --format TEXT change:"
        + change
        + " limit:1",
    ...
    if proc.returncode != 0:
        ...
        log.debug(
            "Change-Id query delivered the following subject result: "
            + commit_information["subject"]
        )
        commit_information["project"] = re.search("project: (.*)\n", out).group(1)
        log.debug(
            "Change-Id query delivered the following project result: "
            + commit_information["project"]
        )
        ...
        for project in projects:
            matches = re.findall(
                issue_pattern % project.key.lower(),
                commit_information["subject"].lower(),
            )
            for m in matches:
...
def generate_text(template, values, commit_information, issue_id):
    return template.format(
        change_owner_username=values["change-owner-username"],
        change_url=GERRIT_URL
        + "/c/"
        + commit_information["project"]
        + "/+/"
        + values["change-url"],
...
def jira_hook(values):
    ...
    a commit-message. The issue-id is extracted from the commig-message by regex.
    """
    log.debug("Executing gerrit-jira-hook...")
    commit_information, issue_ids = find_issue_identifiers(
        values["change"], jira_instance
    )
    if len(issue_ids > 0):
        log.debug('Connecting to Jira-instance running at "' + JIRA_URL + '"...')
        try:
            log.debug('Logging in to Jira as user "' + JIRA_USER + '"...')
            jira_instance = jira.JIRA(
                JIRA_URL, basic_auth=(JIRA_USER, JIRA_PASS), logging=True
            )
            log.debug("Logged in to Jira")
        except requests.exceptions.ConnectTimeout:
            log.error(
                'Connection to "' + JIRA_URL + '" as user "' + JIRA_USER + '" timed out'
            )
            sys.exit(1)
        except (requests.exceptions.ConnectionError, jira.exceptions.JIRAError) as e:
            log.error(
                'Connection to "' + JIRA_URL + '" as user "' + JIRA_USER + '" failed'
            )
            log.error(e)
            sys.exit(1)
        for issue_id in issue_ids:
            log.debug(
                'Generating change information text for issue "' + issue_id + '"...'
            )
            try:
                ...
                if JIRA_USE_FIELD:
                    ...
                    if field_text:
                        for line in field_text.splitlines():
                            if JIRA_LOG_ALL or (
                                not values["project"] in line.split(":")[0]
                            ):
                                field_text_lines.append(line)
                    field_text_lines.append(
                        generate_text(
                            FIELD_TEMPLATE, values, commit_information, issue_id
                        )
                    )
                    field_text = "\n".join(sorted(field_text_lines))
                    issue.update(
                        notify=False, fields={field_map[JIRA_FIELD]: field_text}
                    )
                else:
                    new_change_text = generate_text(
                        COMMENT_TEMPLATE, values, commit_information, issue_id
                    )
                    if not JIRA_LOG_ALL:
                        ...
                        for c in comments:
                            if regex.match(c.body) and re.search(
                                "Project: " + values["project"], c.body
                            ):
